...
import com.example.submissionawalstoryapp.ui.adapter.StoryListAdapter
...
class MainRepositoryTest {
    ...
    fun `verify getStories function is working`() {
        ...
        val expectedStories = MutableLiveData<List<DetailStoryList>>()
    ...
    fun `verify getPagingStory function is working and should not null`() = runTest {
        ...
        val story = MutableLiveData<PagingData<DetailStoryList>>()
        val token = "ini token"
        story.value = data
    ...
    class PagedTestDataSources private constructor() :
        PagingSource<Int, LiveData<List<DetailStoryList>>>() {
        companion object {
            fun snapshot(items: List<DetailStoryList>): PagingData<DetailStoryList> {
                return PagingData.from(items)
            }
        }

        override fun getRefreshKey(state: PagingState<Int, LiveData<List<DetailStoryList>>>): Int {
            return 0
        }

        override suspend fun load(params: LoadParams<Int>): LoadResult<Int, LiveData<List<DetailStoryList>>> {
